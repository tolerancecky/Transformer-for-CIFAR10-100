...
SumWriter = SummaryWriter(log_dir="transformer_100_1_1")
...
class Res_Net(nn.Module):
    def __init__(self, block, num_blocks, num_classes=100):
        ...
        self.layer2 = self._make_layer(block, 132, num_blocks[1], stride=4)
        self.layer3 = self._make_layer(block, 264, num_blocks[2], stride=1)
        self.layer4 = self._make_layer(block, 528, num_blocks[3], stride=2)
        self.linear = nn.Linear(528 * block.expansion, num_classes)
        #self.pool = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
...
parser.add_argument('--outf1', default='./model1_100', help='folder to output images and model checkpoints')
parser.add_argument('--outf2', default='./model2_100', help='folder to output images and model checkpoints')
parser.add_argument('--outf3', default='./model3_100', help='folder to output images and model checkpoints')
parser.add_argument('--outf4', default='./model4_100', help='folder to output images and model checkpoints')# 输出结果保存路径
